...
void FrameTemporalBaseline::UpdateForFrame(const Frame &frame)
{
    ...
    // Decisions for frame t use only state available after frame t-1. This
    // matches the causal timing of object- and MapPoint-carried memories.
    mProbability.copyTo(mDecisionProbability);
...
float FrameTemporalBaseline::ScoreAt(const Frame &frame, float u, float v)
{
    ...
    if(x < 0 || y < 0 || x >= mDecisionProbability.cols || y >= mDecisionProbability.rows)
        return 0.0f;
    return mDecisionProbability.at<float>(y, x);
}
