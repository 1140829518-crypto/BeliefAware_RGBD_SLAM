...
// ===== end P4 log =====
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        // 先根据投影位置更新动态分数，再决定是否允许该点继续参与匹配
        int dynamic_box_id = -1;
        int static_box_id = -1;
        const bool bInDynamicBox = F.IsPixelInDynamicBox(pMP->mTrackProjX, pMP->mTrackProjY, dynamic_box_id)
                                    && !F.IsPixelInStaticBox(pMP->mTrackProjX, pMP->mTrackProjY, static_box_id);
    ...
    for(int i=0; i<LastFrame.N; i++)
    {
        ...
        if(pMP)
        {
            if(!LastFrame.mvbOutlier[i])
            {
                ...
                // Semantic Dynamic Probability Accumulation
                // 当前投影位置若落入动态框，则对该 MapPoint 的动态分数进行累积；否则做缓慢衰减
                int dynamic_box_id = -1;
                int static_box_id = -1;
                const bool bInDynamicBox = CurrentFrame.IsPixelInDynamicBox(u, v, dynamic_box_id)
                                            && !CurrentFrame.IsPixelInStaticBox(u, v, static_box_id);
                ...
                if(bestDist<=TH_HIGH)
                {
                    ...
                    if (CurrentFrame.IsInBox(bestIdx2, box_id)){
                        ...
                        if (SemanticConfig::IsDynamicObjectClass(CurrentFrame.objects_cur_[box_id]->ndetect_class) ){
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        if(pMP)
        {
            ...
            if(!pMP->isBad() && !sAlreadyFound.count(pMP))
            {
                ...
                // Semantic Dynamic Probability Accumulation
                // 当前投影位置命中动态框时，提升该 MapPoint 的动态分数；未命中动态框时，按照帧间隔自然衰减
                int dynamic_box_id = -1;
                int static_box_id = -1;
                const bool bInDynamicBox = CurrentFrame.IsPixelInDynamicBox(u, v, dynamic_box_id)
                                            && !CurrentFrame.IsPixelInStaticBox(u, v, static_box_id);
