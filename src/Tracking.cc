...
inline std::string SemanticClassNameFromId(const int classId)
{
    return SemanticConfig::ClassNameFromId(classId);
}

inline bool BackProjectSemanticObjectCenter(Frame &frame,
                                           ...
                                           cv::Mat &worldCenter)
{
    ...
    const float uCenter = static_cast<float>((bbox[0] + bbox[2]) * 0.5);
    const float vCenter = static_cast<float>((bbox[1] + bbox[3]) * 0.5);
    const int u = static_cast<int>(uCenter);
    const int v = static_cast<int>(vCenter);

    if(u < 0 || v < 0 || u >= frame.imDepth.cols || v >= frame.imDepth.rows)
        return false;

    const float zCenter = frame.imDepth.at<float>(v, u);
    if(zCenter <= 0.0f)
        return false;

    const float x = (uCenter - Frame::cx) * zCenter * Frame::invfx;
    const float y = (vCenter - Frame::cy) * zCenter * Frame::invfy;
    cv::Mat x3Dc = (cv::Mat_<float>(3,1) << x, y, zCenter);
...
void Tracking::Track()
{
    ...
    else//**只存在ＯＫ、ＬＯＳＴ两种可能了，ＯＫ则进行ＳＬＡＭ，ＬＯＳＴ则重定位
    {
        ...
        for (int k=0; k<mCurrentFrame.objects_cur_.size(); ++k){
            // cv::Mat obj_3d;
            if (SemanticConfig::IsStaticObjectClass(mCurrentFrame.objects_cur_[k]->ndetect_class) ){
 ...
 *
 * 说明：
 * - ORBmatcher 阶段负责对 MapPoint 动态分数进行增减
 * - 这里负责在 Tracking 侧统一清理已经超过阈值的 MapPoint
 * - 同时输出每帧动态点数量与被抑制的 MapPoint 数量
 */
void Tracking::CullSemanticDynamicMapPoints()
{
    int nDynamicKeypoints = mCurrentFrame.mnSemanticDynamicKeyPoints;
    int nSuppressedMapPoints = 0;
    ...
    for(const std::shared_ptr<Object> &pObj : mCurrentFrame.objects_cur_)
    {
        if(!pObj)
            continue;
        if(SemanticConfig::IsDynamicObjectClass(pObj->ndetect_class))
            ++nDynamicObjects;
        else if(SemanticConfig::IsStaticObjectClass(pObj->ndetect_class))
            ++nStaticObjects;
    }
    ...
    for(int i=0; i<mCurrentFrame.N; ++i)
    {
        ...
        int dynamic_box_id = -1;
        int static_box_id = -1;
        const cv::KeyPoint &kp = mCurrentFrame.mvKeysUn[i];
        const bool bInDynamicBox = mCurrentFrame.IsPixelInDynamicBox(kp.pt.x, kp.pt.y, dynamic_box_id)
                                   && !mCurrentFrame.IsPixelInStaticBox(kp.pt.x, kp.pt.y, static_box_id);
        const int dynamicClassId = (bInDynamicBox && dynamic_box_id >= 0 && dynamic_box_id < static_cast<int>(mCurrentFrame.objects_cur_.size()) && mCurrentFrame.objects_cur_[dynamic_box_id])
                                 ? mCurrentFrame.objects_cur_[dynamic_box_id]->ndetect_class
                                 : -1;
        if(bInDynamicBox)
            ++nDynamicMapPoints;
    ...
    stats.dynamicObjects = nDynamicObjects;
    stats.staticObjects = nStaticObjects;
...
/**
 * @brief Build and update an object-level semantic map.
 *
 * Semantic Object Landmark construction:
 * - keep the original MapPoint map intact
 * - back-project static / low-dynamic object centers into 3D
 * - associate by class id + 3D distance threshold
 */
void Tracking::UpdateSemanticObjectMap()
{
    int nCreated = 0;
    int nFused = 0;
    int nSkipped = 0;

    for(size_t i = 0; i < mCurrentFrame.objects_cur_.size(); ++i)
    {
        const std::shared_ptr<Object> &pObj = mCurrentFrame.objects_cur_[i];
        if(!pObj)
            continue;

        if(!IsObjectMapCandidate(pObj->ndetect_class))
            continue;

        cv::Mat worldCenter;
        if(!BackProjectSemanticObjectCenter(mCurrentFrame, pObj->vdetect_parameter, worldCenter))
        {
            ++nSkipped;
            continue;
        }

        const std::string className = SemanticClassNameFromId(pObj->ndetect_class);
        SemanticObject* pSemanticObj = new SemanticObject(pObj->ndetect_class,
                                                         className,
                                                         pObj->vdetect_parameter,
                                                         worldCenter,
                                                         mCurrentFrame.mnId);

        SemanticObject* pStored = mpMap->AddOrFuseSemanticObject(pSemanticObj, SemanticConfig::kObjectAssociationDistance);
        if(pStored == pSemanticObj)
            ++nCreated;
        else
            ++nFused;
    }

    cout << "[SemanticObjectMap] frame=" << mCurrentFrame.mnId
         << " created=" << nCreated
         << " fused=" << nFused
         << " skipped=" << nSkipped
         << " total=" << mpMap->SemanticObjectsInMap()
         << endl;
}

void Tracking::SaveSemanticDynamicStatistics(const string &filename)
{
    ofstream f(filename.c_str());
    if(!f.is_open())
    {
        cerr << "[SemanticDynamic] failed to open stats file: " << filename << endl;
        return;
    }
    ...
    for(const SemanticFrameStatistics &stats : mvSemanticFrameStatistics)
    {
        f << stats.frameId << " "
          ...
          << stats.staticObjects << " "
          << stats.totalObjects << endl;
    }
    cout << "[SemanticDynamic] statistics saved to " << filename << endl;
}
