...
/**
 * @brief Semantic Dynamic Probability Accumulation
 *
 * 规则：
 * 1. 目标落入动态框时，动态分数增加
 * 2. 目标连续多帧没有被判定为动态时，动态分数缓慢衰减
 * 3. 通过 frameId 记录帧间隔，避免长时间未观测时的动态证据“冻结”
 */
float MapPoint::UpdateSemanticDynamicScore(const bool &bDynamicHit,
                                           ...
                                           const int &classId)
{
    ...
    const float inc = SemanticConfig::DynamicScoreIncrementForClass(classId);
    const float decay = SemanticConfig::DynamicScoreDecayForClass(classId);
    const unsigned long frameGap = (frameId > mnSemanticDynamicLastFrame) ? (frameId - mnSemanticDynamicLastFrame) : 0;
    if(frameGap > 0)
        mfSemanticDynamicScore *= std::pow(decay, static_cast<float>(frameGap));

    if(bDynamicHit)
        mfSemanticDynamicScore += inc;
    else
        mfSemanticDynamicScore *= decay;

    if(mfSemanticDynamicScore < 0.0f)
...
float MapPoint::GetSemanticDynamicScore()
{
    unique_lock<mutex> lock(mMutexFeatures);
    return mfSemanticDynamicScore;
}

bool MapPoint::ShouldSuppressSemanticDynamic(const int &classId)
{
    if(!SemanticConfig::UseDynamicAccumulation())
        return false;
