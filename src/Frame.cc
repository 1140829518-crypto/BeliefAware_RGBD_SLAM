...
Frame::Frame(const Frame &frame)
    :mpORBvocabulary(frame.mpORBvocabulary), 
     ...
     mnSemanticDynamicKeyPoints(frame.mnSemanticDynamicKeyPoints),
     mnSemanticStaticKeyPoints(frame.mnSemanticStaticKeyPoints)
{
	//逐个复制，其实这里也是深拷贝
     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    //***zt1**step0.5 把检测框添加进Frame中,包含了所有的物体.
    // cout << "**************New Frame *************" << endl;
...
void Frame::UnprojectObjectCenter(const int &box_id , const cv::Mat &imDepth){
    ...
    cv::Mat matCenter3d ;
        ...
        if(!SemanticConfig::IsStaticObjectClass(objects_cur_[box_id]->ndetect_class))
            return;
...
bool Frame::IsPixelInBox(const float &u, const float &v, int &box_id) {
    bool in_box = false;
    for (int k = 0; k < objects_cur_.size(); ++k) {
        const vector<double> box = objects_cur_[k]->vdetect_parameter;
        const double left = box[0];
        const double top = box[1];
        const double right = box[2];
        const double bottom = box[3];
        if (u > left - 2 && u < right + 2 && v > top - 2 && v < bottom + 2) {
            in_box = true;
            box_id = k;
            break;
        }
...
bool Frame::IsPixelInDynamicBox(const float &u, const float &v, int &box_id) {
    bool in_dynamic = false;
    for (int k = 0; k < objects_cur_.size(); ++k) {
        const int obj_class = objects_cur_[k]->ndetect_class;
        if (SemanticConfig::IsDynamicObjectClass(obj_class)) {
            const vector<double> box = objects_cur_[k]->vdetect_parameter;
            const double left = box[0];
            const double top = box[1];
            const double right = box[2];
...
bool Frame::IsPixelInStaticBox(const float &u, const float &v, int &box_id) {
    bool in_static = false;
    for (int k = 0; k < objects_cur_.size(); ++k) {
        const int obj_class = objects_cur_[k]->ndetect_class;
            if (SemanticConfig::IsStaticObjectClass(obj_class)) {
            const vector<double> box = objects_cur_[k]->vdetect_parameter;
            const double left = box[0];
            const double top = box[1];
            const double right = box[2];
