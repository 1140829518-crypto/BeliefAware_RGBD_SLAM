...
cv::Mat FrameDrawer::DrawFrame()
{
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        // Draw semantic detections first.
        for(size_t k=0; k<objects_curFD.size(); ++k)
        {
            if(!objects_curFD[k])
                continue;

            const int classId = objects_curFD[k]->ndetect_class;
            const vector<double> &box = objects_curFD[k]->vdetect_parameter;
            if(box.size() < 4)
                continue;

            const cv::Point pt11(cvRound(box[0]), cvRound(box[1]));
            const cv::Point pt22(cvRound(box[2]), cvRound(box[3]));
            const cv::Point center(cvRound((box[0] + box[2]) * 0.5), cvRound((box[1] + box[3]) * 0.5));

            cv::Scalar boxColor(0, 180, 180);
            cv::Scalar centerColor(0, 180, 180);
            if(SemanticConfig::IsDynamicObjectClass(classId))
            {
                boxColor = cv::Scalar(0, 0, 255);
                centerColor = cv::Scalar(0, 0, 255);
            }
            else if(SemanticConfig::IsStaticObjectClass(classId))
            {
                boxColor = cv::Scalar(0, 220, 0);
                centerColor = cv::Scalar(255, 255, 0);
            }

            cv::rectangle(im, pt11, pt22, boxColor, 2);
            cv::circle(im, center, 3, centerColor, -1);
            cv::putText(im, SemanticConfig::ClassNameFromId(classId), pt11,
                        cv::FONT_HERSHEY_PLAIN, 0.8, boxColor, 1);
        }
...
void FrameDrawer::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText)
{
    ...
    else if(nState==Tracking::OK)
    {
        ...
        const size_t nSemanticObjects = mpMap->SemanticObjectsInMap();
        s << "KFs: " << nKFs << ", MPs: " << nMPs << ", SOs: " << nSemanticObjects << ", Matches: " << mnTracked;
        //在视觉里程计中匹配到的
        if(mnTrackedVO>0)
