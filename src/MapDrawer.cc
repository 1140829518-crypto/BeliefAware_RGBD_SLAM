...
void MapDrawer::DrawObjStatic(){
    ...
    const vector<SemanticObject*> vpSemanticObjects = mpMap->GetAllSemanticObjects();
    if(vpSemanticObjects.empty())
        return;

    glPointSize(mPointSize * 2.0f);
    glBegin(GL_POINTS);
    glColor3f(1.0f,0.7f,0.0f);
    for(size_t i = 0, iend = vpSemanticObjects.size(); i < iend; ++i)
    {
        if(!vpSemanticObjects[i])
            continue;
        cv::Mat pos = vpSemanticObjects[i]->GetWorldCenter();
        if(pos.empty())
            continue;
