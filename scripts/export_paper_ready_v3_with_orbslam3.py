#!/usr/bin/env python3
"""Export paper-ready tables with ORB-SLAM3 comparison into a new directory."""
...
OUT_DIR = Path("paper_ready_outputs_v3")
...
def add_orb_slam3_rows(rows: Dict[Tuple[str, str], Dict[str, str]]) -> None:
    for sequence, _ in SEQUENCES:
        metrics_path = OUT_DIR / "orb_slam3_eval" / sequence / "metrics.json"
        if not metrics_path.exists():
            continue
        data = json.loads(metrics_path.read_text(encoding="utf-8"))
        rows[(sequence, "ORB-SLAM3")] = {
            "sequence": sequence,
            "method": "ORB-SLAM3",
            "source": "ORB_SLAM3_trajectory",
...
def write_markdown(path: Path, title: str, table: List[List[str]]) -> None:
    ...
    lines.append("")
    lines.append("Note: **bold** marks the best available value in each column. `--` means no local trajectory/runtime is available.")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_merged_summary(path: Path, rows: Dict[Tuple[str, str], Dict[str, str]]) -> None:
    fieldnames = [
        "sequence",
        "method",
        "source",
...
def draw_pipeline(path_prefix: Path) -> None:
    ...
    for i, ((title, subtitle), color) in enumerate(zip(steps, colors)):
        x = x0 + i * (w + gap)
        box = FancyBboxPatch((x, y0), w, h, boxstyle="round,pad=0.018,rounding_size=0.018", linewidth=1.1, edgecolor="#333333", facecolor=color)
        ax.add_patch(box)
        ...
        if i < len(steps) - 1:
            ax.add_patch(FancyArrowPatch((x + w + 0.004, y0 + h / 2), (x + w + gap - 0.006, y0 + h / 2), arrowstyle="-|>", mutation_scale=12, linewidth=1.1, color="#333333"))
...
def main() -> None:
    ...
    add_orb_slam3_rows(rows)
    write_merged_summary(OUT_DIR / "summary_with_orb_slam3.csv", rows)
    ...
    export_figures()
    print(f"Wrote paper-ready outputs with ORB-SLAM3 to {OUT_DIR}")
