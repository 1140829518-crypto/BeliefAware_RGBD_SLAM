...
public:
    ...
    /**
     * @brief 语义动态概率累积更新
     * @details Semantic Dynamic Probability Accumulation:
     *          1) 目标进入动态框时加分
     *          2) 连续多帧没有动态证据时缓慢衰减
     *          3) 返回更新后的动态分数
     */
    float UpdateSemanticDynamicScore(const bool &bDynamicHit, const long unsigned int &frameId,
                                     const int &classId = -1);

    /**
     * @brief 获取当前语义动态分数
     */
    float GetSemanticDynamicScore();
