...
public:
    ...
    //*************
    // 根据像素坐标判断是否落入检测框
    bool IsPixelInBox(const float &u, const float &v, int &box_id);
    // 根据像素坐标判断是否落入动态类别框
    bool IsPixelInDynamicBox(const float &u, const float &v, int &box_id);
    // 根据像素坐标判断是否落入静态类别框
    bool IsPixelInStaticBox(const float &u, const float &v, int &box_id);
    //判断特征点是否在物体框内
...
public:
    ...
    // Semantic statistics for Semantic Dynamic Probability Accumulation
    int mnSemanticDynamicKeyPoints = 0;
    int mnSemanticStaticKeyPoints = 0;
