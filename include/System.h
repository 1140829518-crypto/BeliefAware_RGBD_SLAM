...
public:
    ...
    void Shutdown();
    ORB_SLAM2::Frame GetTrackingFrame(); //zt为了保存图片新增
    ...
    // Save semantic object landmarks.
    void SaveSemanticObjects(const string &filename);

    // Save semantic dynamic statistics.
    void SaveSemanticDynamicStatistics(const string &filename);
    ...
    // 以KITTI格式保存相机的运行轨迹
    void SaveKeyFrameTrajectoryEuroc(const string &filename); //zt    //指定文件名
